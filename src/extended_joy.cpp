...
bool TeleopTwistJoyExtended::Impl::returnSwitchStateFromPush(bool state) {
  if (ros::Time::now().toSec() - _time_old.toSec() <
...
void TeleopTwistJoyExtended::Impl::sendExtnDataMsg(
  ...
  extn_data_msg.extn_data.inverse_movement =
      returnSwitchStateFromPush(joy_msg->buttons[inverse_movement]);
  extn_data_msg.extn_data.left_light_bulb =
      (joy_msg->axes[left_light_bulb] == -1 ? true : false);
