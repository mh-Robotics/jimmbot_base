...
HilWheelCommander::HilWheelCommander() {
  esp32_can_sub_ = node_.subscribe<sensor_msgs::JointState>(
      SUBSCRIBER_COMMAND_TOPIC, 1, &HilWheelCommander::esp32NodeCallback, this);
  extn_data_sub_ = node_.subscribe<jimmbot_msgs::ExtnDataStamped>(
      SUBSCRIBER_EXTN_DATA_TOPIC, 1, &HilWheelCommander::extnDataMsgCallback,
      this);
  esp32_can_feedback_sub_ = node_.subscribe<jimmbot_msgs::CanFrameStamped>(
      kFeedbackTopicCanMsg, 1, &HilWheelCommander::esp32NodeFeedbackCallback,
      this);
  esp32_can_pub_ = node_.advertise<jimmbot_msgs::CanFrameStamped>(
      kCommandTopicCanMsg, 1, false);
}

bool HilWheelCommander::negativeBit(const double& speed) const {
  //@todo check here. Fix the firmware to have the reverse logic here. For
  // testing, this is OK.
  return speed < 0 ? false : true;
}

uint8_t HilWheelCommander::regulateSpeedToUInt8(const double& speed) const {
  return (std::abs(this->angularToLinear(speed)) * UINT8_MAX /
          this->getMaxSpeed());
}
...
void HilWheelCommander::esp32NodeCallback(
  ...
  for (int index = 0; index < CAN_MSG_COUNT; index++) {
    ...
    _data_frame.can_frame.data = {
        ...
        0,
        0,
        negativeBit(state->velocity[index + 2]),
        regulateSpeedToUInt8(state->velocity[index + 2])};
...

  {
    _data_frame.header.stamp = ros::Time::now();
    _data_frame.header.frame_id = "/jimmbot/hw/hil/cmd";
    _data_frame.can_frame.data = {0, 0, 0, this->lights_.first,
                                  0, 0, 0, this->lights_.second};
...
void HilWheelCommander::extnDataMsgCallback(
    const jimmbot_msgs::ExtnDataStamped::ConstPtr& extn_data_msg) {
  this->lights_ = {extn_data_msg->extn_data.left_light_bulb,
...
void HilWheelCommander::esp32NodeFeedbackCallback(
    const jimmbot_msgs::CanFrameStamped::ConstPtr& data_msg) {
  ...
  ROS_WARN("Position: %.2f",
           regulatePositionToDouble(data_msg->can_frame.data[2],
                                    data_msg->can_frame.data[3]));
  ROS_WARN("RPM: %d", regulateRpmToInt(data_msg->can_frame.data[4],
                                       data_msg->can_frame.data[5]));
  ROS_WARN("Speed: %.2f", regulateSpeedToDouble(data_msg->can_frame.data[6],
                                                data_msg->can_frame.data[7]));
}
